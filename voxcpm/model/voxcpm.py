...
class VoxCPMModel(nn.Module):
    ...
    def build_prompt_cache(
        ...
        prompt_text: str,
        prompt_wav_path: str,
        max_prompt_duration: float = 5.0,
        use_prompt_text: bool = True,
        cross_language: bool = False,  # 新增：是否为跨语言场景
    ):
        """
        ...
        Args:
            ...
            max_prompt_duration: maximum duration of prompt audio in seconds (default: 5.0)
            use_prompt_text: whether to use prompt text (default: True)
            cross_language: whether this is cross-language voice cloning (default: False)
                           If True, will use separator to reduce content leakage
            
        Returns:
            prompt_cache: dict with prompt_text, audio features, use_prompt_text flag, and cross_language flag.
        """
        if not prompt_wav_path:
            raise ValueError("prompt_wav_path is required")
        # If text is not provided, use an empty string
        ...
        # 限制参考音频长度，避免过长的 prompt 导致生成异常
        max_samples = int(max_prompt_duration * self.sample_rate)
        if audio.size(1) > max_samples:
            # 从音频中间截取，保留最有代表性的部分
            start_idx = (audio.size(1) - max_samples) // 2
            audio = audio[:, start_idx:start_idx + max_samples]
            print(f"  参考音频过长，已截取至 {max_prompt_duration}秒", file=sys.stderr)
    ...
    def _generate_with_prompt_cache(
        ...
        if prompt_cache is None:
            ...
            # 改进：支持三种模式
            # 1. 标准模式：直接拼接（同语言，最佳音色）
            # 2. 缩短模式：只用最后几个词（跨语言，平衡音色和内容）
            # 3. 仅音频模式：不使用参考文本（最后的选择）
            use_prompt_text = prompt_cache.get("use_prompt_text", True)
            cross_language = prompt_cache.get("cross_language", False)
            
            if use_prompt_text and prompt_text and prompt_text.strip():
                if cross_language:
                    # 跨语言场景：只保留参考文本的最后3-5个词
                    # 这样既能提供音色信息，又能大幅减少内容泄露
                    words = prompt_text.strip().split()
                    if len(words) > 5:
                        # 只保留最后5个词
                        shortened_prompt = ' '.join(words[-5:])
                        text = shortened_prompt + "。 " + target_text
                        print(f"  [INFO] 跨语言模式，缩短参考文本: {len(words)}词 → 5词", file=sys.stderr)
                    else:
                        # 参考文本本身就很短，使用分隔符
                        text = prompt_text.strip() + "。 " + target_text
                        print(f"  [INFO] 跨语言模式，使用分隔符", file=sys.stderr)
                else:
                    # 同语言场景：标准拼接
                    text = prompt_text + " " + target_text
            else:
                ...
                if not use_prompt_text:
                    print(f"  [INFO] 使用仅音频模式，不包含参考文本", file=sys.stderr)
